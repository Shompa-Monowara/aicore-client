    ...
    <nav className="sticky top-0 z-50 w-full border-b border-purple-950/30 bg-[#0b0813]/70 backdrop-blur-md">
      <header className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <ul className="hidden items-center gap-8 md:flex">
          ...
          <li>
            <Link href="/all-prompts" className="text-sm font-medium text-zinc-400 transition-colors hover:text-white">
              All Prompts
            </Link>
          </li>
        </ul>
        ...
        <div className="hidden items-center gap-4 md:flex">
          {!user ? (
            <>
              <Link 
                href="/auth/login" 
                className="flex items-center gap-1 rounded-xl border border-zinc-800 bg-zinc-900/40 px-4 py-2 text-sm font-medium text-white transition-all hover:bg-zinc-800 hover:border-zinc-700"
              >
                <LogOutIcon className="rotate-180 text-lg" />
                Login
              </Link>
                ...
                <Dropdown.Popover>
                  <div className="px-3 pt-3 pb-1 bg-[#13112b] border-b border-purple-950/40 text-white rounded-t-xl">
                    <div className="flex items-center gap-2">
                      <Avatar size="sm" className="border border-purple-500/20">
                      ...
                      <div className="flex flex-col gap-0">
                        ...
                        <p className="text-xs leading-none text-zinc-400 mt-0.5">
                          {user?.email}
                        </p>
                      </div>
                    </div>
                  ...
                  <Dropdown.Menu
                    className="bg-[#13112b] text-zinc-300 rounded-b-xl"
      ...
      {isMenuOpen && (
        <div className="border-t border-purple-950/20 bg-[#0b0813] md:hidden">
          <ul className="flex flex-col gap-2 p-4 text-zinc-400">
            <li>
              <Link href="/" className="block py-2 text-sm font-medium hover:text-white" onClick={() => setIsMenuOpen(false)}>
                Home
              </Link>
            </li>
            <li>
              <Link href="/prompts" className="block py-2 text-sm font-medium hover:text-white" onClick={() => setIsMenuOpen(false)}>
                All Prompts
              </Link>
            </li>
            
            {!user ? (
              <li className="mt-4 flex flex-col gap-2 border-t border-zinc-800/60 pt-4">
                <Link 
                  href="/auth/login" 
                  className="flex h-10 items-center justify-center rounded-xl border border-zinc-800 bg-zinc-900/40 text-sm font-medium text-white"
                  onClick={() => setIsMenuOpen(false)}
                >
                  Login
                </Link>
                <Link href="/auth/register" onClick={() => setIsMenuOpen(false)}>
                  <Button className="w-full rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 font-semibold text-white">
                    Register
                  </Button>
                </Link>
              </li>
            ) : (
              <li className="mt-4 border-t border-zinc-800/60 pt-4">
                <Link href={`/dashboard/${user?.role || "user"}`} className="block py-2 text-sm" onClick={() => setIsMenuOpen(false)}>
                  Dashboard
                </Link>
                <button onClick={() => { handleSignOut(); setIsMenuOpen(false); }} className="block w-full py-2 text-left text-sm text-rose-500">
                  Logout
                </button>
              </li>
            )}
